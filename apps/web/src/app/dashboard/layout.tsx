...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className={`flex h-screen overflow-hidden font-sans ambient-bg ${isDarkMode ? 'dark bg-[#09090b] text-zinc-100' : 'bg-[#f5f5f7] text-slate-900'}`}>
      
      {/* ── Desktop Floating 24px Executive Sidebar ── */}
      <motion.aside
        initial={false}
        animate={{ width: isCollapsed ? 80 : 260 }}
        transition={{ duration: 0.3, ease: [0.16, 1, 0.3, 1] }}
        className="hidden md:flex flex-col m-3 rounded-[24px] exec-card relative z-30 overflow-hidden"
      >
        {/* Brand Header */}
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className={`relative flex items-center space-x-3 px-3.5 py-2.5 rounded-2xl text-sm font-semibold transition-all group ${
                  isActive
        ...
        {/* Workspace Card Inspired by Reference */}
        {!isCollapsed && (
          <div className="p-3.5 mx-3 mb-3 rounded-2xl bg-gradient-to-br from-rose-50 to-indigo-50 dark:from-rose-950/30 dark:to-indigo-950/20 border border-rose-100 dark:border-rose-500/20 text-center space-y-1.5">
        ...
        <div className="p-3 border-t border-slate-200/60 dark:border-white/10 bg-slate-50/50 dark:bg-zinc-950/40">
          <div className="flex items-center justify-between p-2 rounded-2xl bg-white dark:bg-white/5 border border-slate-200/60 dark:border-white/5 shadow-sm">
